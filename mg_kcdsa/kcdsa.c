...
unsigned int KISA_KCDSA_CreateObject(KISA_KCDSA** kcdsa) {
    ...
    *kcdsa = BN_Key = (KISA_KCDSA*)malloc(sizeof(KISA_KCDSA));
    ...
    if((BN_Key->KCDSA_P = CreateBigNum(tt + 1)) == NULL)
        goto LABEL_END0;
    if((BN_Key->KCDSA_G = CreateBigNum(tt + 1)) == NULL)
        goto LABEL_END0;
    if((BN_Key->KCDSA_y = CreateBigNum(tt + 1)) == NULL)
        goto LABEL_END0;
    ...
    if((BN_Key->KCDSA_Q = CreateBigNum(tt + 1)) == NULL)
        goto LABEL_END0;
    if((BN_Key->KCDSA_x = CreateBigNum(tt + 1)) == NULL)
        goto LABEL_END0;
    for(i = 0; i < (signed)(tt + 1); i++) {
...
unsigned int KCDSA_PRNG_SHA_224(
    ...
    unsigned char Count = 0, DigestValue[SHA224_DIGEST_VALUELEN] = {
                                 0,
                             };
    unsigned int i;
...
unsigned int KCDSA_PRNG_SHA_256(
    ...
    unsigned char* pbDst,
    unsigned int dDstBitLen) {
    unsigned char Count = 0, DigestValue[SHA256_DIGEST_VALUELEN] = {
                                 0,
                             };
    unsigned int i = 0;
    unsigned int tempLen = dSrcByteLen;
...
unsigned int Generate_Random(
    ...
    if((VAL = CreateBigNum(XBitLen / 32)) == NULL)
        goto LABEL_END;
    if((BN_Tmp1 = CreateBigNum(XBitLen / 32)) == NULL)
        goto LABEL_END;
    ...
    if(HASH == SHA224) {
        ret = KCDSA_PRNG_SHA_224(&SHA224_AlgInfo, pbSrc, dSrcByteLen, bzTmp1, XBitLen);
        GOTO_END;
    } else if(HASH == SHA256) {
        ret = KCDSA_PRNG_SHA_256(&SHA256_AlgInfo, pbSrc, dSrcByteLen, bzTmp1, XBitLen);
        GOTO_END;
    }

    ret = OS2BN(bzTmp1, XBitLen / 8, BN_Tmp1);
    GOTO_END;

    ret = BN_Add(VAL, XKEY, BN_Tmp1);
    GOTO_END;
    ...
    ret = BN2OS(VAL, VAL->Length * 4, bzTmp1);
    GOTO_END;

    if(HASH == SHA224) {
        ret = KCDSA_PRNG_SHA_224(&SHA224_AlgInfo, bzTmp1, VAL->Length * 4, bzTmp2, XBitLen);
        GOTO_END;
    } else {
        ret = KCDSA_PRNG_SHA_256(&SHA256_AlgInfo, bzTmp1, VAL->Length * 4, bzTmp2, XBitLen);
        GOTO_END;
    }

    ret = OS2BN(bzTmp2, XBitLen / 8, BN_Tmp1);
    GOTO_END;

    while(BN_Cmp(BN_Tmp1, kcdsa->KCDSA_Q) >= 0) {
        ret = BN_Sub(BN_Tmp1, BN_Tmp1, kcdsa->KCDSA_Q);
        GOTO_END;
    }
...
unsigned int KISA_KCDSA_GenerateKeyPair(
    ...
    if((BN_Tmp1 = CreateBigNum(i + 1)) == NULL)
        goto LABEL_END;
    if((XKEY = CreateBigNum(qLen / 32)) == NULL)
        goto LABEL_END;

    if(KCDSA_Key->KCDSA_x->Length == 0) {
        // b 비트의 임의의 정수 XKEY 생성
        srand((unsigned)time(NULL));
        ret = BN_Rand(XKEY, qLen);
        GOTO_END;
    ...
    ret = BN_ModExp(KCDSA_Key->KCDSA_y, KCDSA_Key->KCDSA_G, BN_Tmp1, KCDSA_Key->KCDSA_P);
    GOTO_END;
...
unsigned int KISA_KCDSA_sign(
    ...
    if(Signature == NULL)
        return CTR_INVALID_POINTER; // 사인이 NULL이면 오류

    if(MsgDigest == NULL)
        return CTR_INVALID_POINTER; // MsgDigest가 NULL이면 오류

    ret = CTR_MEMORY_ALLOC_ERROR;
    // P의 크기만큼  K와 Tmp1에 공간 할당
    i = kcdsa->KCDSA_P->Length;
    if((BN_K = CreateBigNum(i + 1)) == NULL)
        goto LABEL_END;
    if((BN_Tmp1 = CreateBigNum(i + 1)) == NULL)
        goto LABEL_END;
    // Q의 크기만큼 S에 공간 할당
    i = kcdsa->KCDSA_Q->Length;
    if((KCDSA_s = CreateBigNum(i + 1)) == NULL)
        goto LABEL_END;
    if((KKEY = CreateBigNum(i + 1)) == NULL)
        goto LABEL_END;

    // step 1. 난수 k를 [1, Q-1]에서 임의로 선택한다.
    srand((unsigned)time(NULL));
    ret = BN_Rand(KKEY, 8 * qByteLen);
    GOTO_END;
    ...
    if(BN_Cmp(BN_K, kcdsa->KCDSA_Q) >= 0) {
        ret = BN_Sub(BN_K, BN_K, kcdsa->KCDSA_Q);
        GOTO_END;
    }

    // step 2. W=G^K mod P를 계산한다.
    ret = BN_ModExp(BN_Tmp1, kcdsa->KCDSA_G, BN_K, kcdsa->KCDSA_P);
    GOTO_END;

    //	step 3. 서명의 첫 부분 R=h(W)를 계산한다.
    i = DIGITSIZE * kcdsa->KCDSA_P->Length; // 바이트 단위 길이
    ret = BN2OS(BN_Tmp1, i, bzTmp);
    GOTO_END;
    j = i;
    if(HASH == SHA224) {
        SHA224_Init(&SHA224_AlgInfo);
    ...
    ret = BN2OS(kcdsa->KCDSA_y, 512 / 8, bzTmp1);
    GOTO_END;
    ...
    if((HASH == SHA224 && qByteLen == 28) || (HASH == SHA256 && qByteLen == 32))
        ...
        for(i = 0; i < qByteLen; i++)
            bzTmp[i + 4] ^= hashTmp[i + 4];
        for(i = 0; i < qByteLen; i++)
            bzTmp[i] = bzTmp[i + 4];
    }
    ...
    ret = OS2BN(bzTmp, i, BN_Tmp1);
    GOTO_END;
    ret = BN_ModRed(BN_Tmp1, BN_Tmp1, kcdsa->KCDSA_Q);
    GOTO_END;

    //	step 7. S = X(K-E) mod Q를 계산한다.
    ret = BN_ModSub(BN_K, BN_K, BN_Tmp1, kcdsa->KCDSA_Q);
    GOTO_END;
    ret = BN_ModMul(KCDSA_s, kcdsa->KCDSA_x, BN_K, kcdsa->KCDSA_Q);
    GOTO_END;

    ret = BN2OS(KCDSA_s, qByteLen, Signature + qByteLen);
    GOTO_END;
...
unsigned int KISA_KCDSA_verify(
    ...
    if((SignLen != DigestLen + qByteLen) && (SignLen != qByteLen + qByteLen))
        return CTR_INVALID_SIGNATURE_LEN;

    ret = CTR_MEMORY_ALLOC_ERROR;
    i = kcdsa->KCDSA_P->Length;
    if((BN_Tmp1 = CreateBigNum(i)) == NULL)
        goto LABEL_END;
    if((BN_Tmp2 = CreateBigNum(i)) == NULL)
        goto LABEL_END;
    if((BN_Tmp3 = CreateBigNum(i)) == NULL)
        goto LABEL_END;
    i = kcdsa->KCDSA_Q->Length;
    if((KCDSA_s = CreateBigNum(i)) == NULL)
        goto LABEL_END;

    memcpy(bzTmp, Signature, qByteLen);
    ret = OS2BN(Signature + qByteLen, qByteLen, KCDSA_s);
    GOTO_END;

    ret = CTR_VERIFY_FAIL;
    if(BN_Cmp(KCDSA_s, kcdsa->KCDSA_G) >= 0)
        goto LABEL_END;
    ...
    ret = BN_ModExp(BN_Tmp2, kcdsa->KCDSA_y, KCDSA_s, kcdsa->KCDSA_P);
    GOTO_END;
    ret = BN_ModExp(BN_Tmp3, kcdsa->KCDSA_G, BN_Tmp1, kcdsa->KCDSA_P);
    GOTO_END;
    ret = BN_ModMul(BN_Tmp1, BN_Tmp2, BN_Tmp3, kcdsa->KCDSA_P);
    GOTO_END;
...
LABEL_END:
    if(BN_Tmp1 != NULL)
        DestroyBigNum(BN_Tmp1);
    if(BN_Tmp2 != NULL)
        DestroyBigNum(BN_Tmp2);
    if(BN_Tmp3 != NULL)
        DestroyBigNum(BN_Tmp3);
    if(KCDSA_s != NULL)
        DestroyBigNum(KCDSA_s);
    return ret;
}
...
unsigned int KISA_KCDSA_GenerateParameters(
    ...
    unsigned int HASH) {
    int j;
    unsigned char bzTmp[3072 / 8 + 4] = {
        0,
    };
    unsigned char tSeed[256 / 8 + 4] = {
        0x00,
    };
    /*p = 2048, q = 224, SHA-224 test vector*/
    ...
    ret = CTR_MEMORY_ALLOC_ERROR;
    if((BN_Tmp1 = CreateBigNum(PrimeBits / 32 + 1)) == NULL)
        goto LABEL_END;
    if((BN_Tmp2 = CreateBigNum(PrimeBits / 32 + 1)) == NULL)
        goto LABEL_END;
...
LABEL_Start:
    for(;;) {
        ...
        if(HASH == SHA224) {
            ret = KCDSA_PRNG_SHA_224(&SHA224_AlgInfo, tSeed, kcdsa->SeedLen, bzTmp, PrimeBits - SubPrimeBits - 4);
            GOTO_END;
        } else {
            ret = KCDSA_PRNG_SHA_256(&SHA256_AlgInfo, tSeed, kcdsa->SeedLen, bzTmp, PrimeBits - SubPrimeBits - 4);
            GOTO_END;
        }

        ret = OS2BN(bzTmp, (PrimeBits - SubPrimeBits) / 8, BN_Tmp1);
        GOTO_END;
    ...
    if((KCDSA_J = CreateBigNum(PrimeBits / 32 + 1)) == NULL)
        ;
    GOTO_END;
    ...
    for(Count = 1; Count < (1 << 24); Count++) {
        ...
        if(HASH == SHA224) {
            ret = KCDSA_PRNG_SHA_224(&SHA224_AlgInfo, tSeed, kcdsa->SeedLen + 4, bzTmp, SubPrimeBits);
            GOTO_END;
        } else {
            ret = KCDSA_PRNG_SHA_256(&SHA256_AlgInfo, tSeed, kcdsa->SeedLen + 4, bzTmp, SubPrimeBits);
            GOTO_END;
        }

        ret = OS2BN(bzTmp, SubPrimeBits / 8, kcdsa->KCDSA_Q);
        GOTO_END;
    ...
    for(;;) {
        ...
        ret = BN_ModExp(kcdsa->KCDSA_G, BN_Tmp2, KCDSA_J, kcdsa->KCDSA_P);
        GOTO_END;
...
unsigned int KISA_KCDSA_DestroyObject(KISA_KCDSA** kcdsa) {
    ...
    if(BN_Key->KCDSA_P != NULL)
        DestroyBigNum(BN_Key->KCDSA_P);
    if(BN_Key->KCDSA_Q != NULL)
        DestroyBigNum(BN_Key->KCDSA_Q);
    if(BN_Key->KCDSA_G != NULL)
        DestroyBigNum(BN_Key->KCDSA_G);
    if(BN_Key->KCDSA_x != NULL)
        DestroyBigNum(BN_Key->KCDSA_x);
    if(BN_Key->KCDSA_y != NULL)
        DestroyBigNum(BN_Key->KCDSA_y);
