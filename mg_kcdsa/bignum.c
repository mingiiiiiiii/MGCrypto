...
#define CheckInput_MemLen(L) \
    if(L->Space < L->Length) \
    return ERROR_MemLen1
#define CheckOutput_MemLen(L) \
    if(L->Space < L->Length)  \
    return ERROR_MemLen2
...
/*************** Prototypes ***********************************************/
unsigned int Classical_REDC(unsigned int* L_Dst,
                            unsigned int DstLen,
                            unsigned int* L_Modulus,
                            unsigned int ModLen);
unsigned int Montgomery_Init(unsigned int* L_Modulus,
                             unsigned int ModLen);
unsigned int Montgomery_REDC(unsigned int* L_Dst,
                             unsigned int DstLen,
                             unsigned int* L_Modulus,
                             unsigned int ModLen);
unsigned int Montgomery_Zn2RZn(unsigned int* L_Dst,
                               unsigned int* L_Src,
                               unsigned int* L_Modulus,
                               unsigned int ModLen);
 ...
 */
static void D_Mul_D(unsigned int* D_Res,
                    unsigned int D_Multiplicand,
                    unsigned int D_Multiplier) {
#if defined(DOUBLE_DIGIT)
    DOUBLE_DIGIT tmp;

    tmp = (DOUBLE_DIGIT)D_Multiplicand * D_Multiplier;
    D_Res[0] = (unsigned int)tmp;
    D_Res[1] = (unsigned int)(tmp >> BitsInDIGIT);
#else
    unsigned int Da, Db, R1, R0;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static unsigned int DD_Div_D(unsigned int D_Dividend1,
                             unsigned int D_Dividend2,
                             unsigned int D_Divisor) {

#if defined(DOUBLE_DIGIT)
    return (unsigned int)(((((DWORD)D_Dividend1) << BitsInDIGIT) + D_Dividend2) / D_Divisor);
#else

    unsigned int Dq, Dr, Dx, Dy, Dt;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static unsigned int DD_Mod_D(unsigned int D_Dividend1,
                             unsigned int D_Dividend2,
                             unsigned int D_Divisor) {
#if defined(DOUBLE_DIGIT)
    return (unsigned int)(((((DWORD)D_Dividend1) << BitsInDIGIT) + D_Dividend2) % D_Divisor);
#else
    unsigned int Dq, Dr, Dx, Dy;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static unsigned int D_Gcd(unsigned int D_Src1,
                          unsigned int D_Src2) {
    unsigned int tmp;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static unsigned int bn_ModD(unsigned int* L_Src,
                            unsigned int SrcLen,
                            unsigned int D_Divisor) {
    unsigned int i;
    unsigned int xx = 0;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static int bn_Cmp(unsigned int* L_Src1,
                  unsigned int SrcLen1,
                  unsigned int* L_Src2,
                  unsigned int SrcLen2) {
    unsigned int i;
    ...
    for(; i != (unsigned int)-1; i--) {
        if(L_Src1[i] == L_Src2[i])
            continue;
        else if(L_Src1[i] > L_Src2[i])
            return +1;
        else
            return -1;
    }
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static void bn_Copy(unsigned int* L_Dst,
                    unsigned int* L_Src,
                    unsigned int SrcLen) {
    unsigned int i;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static unsigned int bn_SHL(unsigned int* L_Dst,
                           unsigned int* L_Src,
                           unsigned int SrcLen,
                           unsigned int NumOfShift) {
    unsigned int i = SrcLen - 1;
    unsigned int ret;

    ret = L_Src[i] >> (BitsInDIGIT - NumOfShift);
    for(; i != 0; i--)
        L_Dst[i] = (L_Src[i] << NumOfShift) ^ (L_Src[i - 1] >> (BitsInDIGIT - NumOfShift));
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static unsigned int bn_SHR(unsigned int* L_Dst,
                           unsigned int* L_Src,
                           unsigned int SrcLen,
                           unsigned int NumOfShift) {
    unsigned int i = 0;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static unsigned int bn_Add(unsigned int* L_Dst,
                           unsigned int* L_Src1,
                           unsigned int SrcLen1,
                           unsigned int* L_Src2,
                           unsigned int SrcLen2) {
    ...
    for(carry = i = 0; i < SrcLen2; i++) {
        if((L_Src2[i] == ((unsigned int)-1)) && (carry == 1))
            L_Dst[i] = L_Src1[i];
        else {
            tmp = L_Src2[i] + carry;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static unsigned int bn_Sub(unsigned int* L_Dst,
                           unsigned int* L_Src1,
                           unsigned int SrcLen1,
                           unsigned int* L_Src2,
                           unsigned int SrcLen2) {
 ...
 *	** Assume SrcLen>0 **
 */
static unsigned int bn_MulD(unsigned int* L_Dst,
                            unsigned int* L_Src,
                            unsigned int SrcLen,
                            unsigned int D_Multiplier) {
    unsigned int i;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static unsigned int bn_MulAdd(unsigned int* L_Dst,
                              unsigned int DstLen,
                              unsigned int* L_Src,
                              unsigned int SrcLen,
                              unsigned int D_Multiplier) {
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static unsigned int bn_MulSub(unsigned int* L_Dst,
                              unsigned int DstLen,
                              unsigned int* L_Src,
                              unsigned int SrcLen,
                              unsigned int D_Multiplier) {
 ...
 *	** Assume SrcLen>1 **
 */
static void bn_Sqr(unsigned int* L_Dst,
                   unsigned int* L_Src,
                   unsigned int SrcLen) {
    unsigned int i, j;
    unsigned int tmp, La[2];
    ...
    for(tmp = i = j = 0; i < SrcLen; i++, j += 2) {
        ...
        if((La[0] += tmp) < tmp)
            La[1]++;
        if((L_Dst[j] += La[0]) < La[0])
            La[1]++;
        if((L_Dst[j + 1] += La[1]) < La[1])
            tmp = 1;
        else
            tmp = 0;
    }
 ...
 */
static void bn_Mul(unsigned int* L_Dst,
                   unsigned int* L_Src1,
                   unsigned int SrcLen1,
                   unsigned int* L_Src2,
                   unsigned int SrcLen2) {
 ...
 *	** Assume SrcLen>1 **
 */
static void bn_KaraSqr(unsigned int* L_Dst,
                       unsigned int* L_Src,
                       unsigned int SrcLen) {
    int FLAG = 0;
 ...
 *	** Assume SrcLen > 1 **
 */
static void bn_KaraMul(unsigned int* L_Dst,
                       unsigned int* L_Src1,
                       unsigned int* L_Src2,
                       unsigned int SrcLen) {
    int FLAG = 0, SIGN = 0;
 ...
 */
static unsigned int bn_Div(unsigned int* L_Dst,
                           unsigned int* L_Rem,
                           unsigned int* L_Src1,
                           unsigned int SrcLen1,
                           unsigned int* L_Src2,
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static unsigned int bn_Euclid(unsigned int* L_Dst,
                              unsigned int* L_Src,
                              unsigned int* L_Modulus,
                              unsigned int ModLen) {
...
// static unsigned int	Window_PreData2[1<<(W_size4DoubleExp-1)][MaxDIGIT+1];
// static int		Add_Chain2[BN_MAX_BITS/W_size4DoubleExp][2];
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
static unsigned int bn_ModExp(unsigned int* L_Dst,
                              unsigned int* L_Base,
                              ...
                              unsigned int ModLen) {
    unsigned int ret;
    unsigned int i, j, MSB = 0, WindowSize;
    ...
    if(i == (unsigned int)-1) { //	L_Exponent[] = 0
        L_Dst[0] = 1;
        for(j = 1; j < (int)ModLen; j++)
            L_Dst[j] = 0;
        //
        ret = CTR_SUCCESS;
        goto LABEL_END;
    ...
    if(i < 32)
        ...
        WindowSize = 7;
    if(WindowSize > Max_W_size)
        WindowSize = Max_W_size;
    ...
    if(WindowSize == 1) {
        ...
        ret = Algorithm_REDC(L_Temp2, 2 * ModLen, L_Modulus, ModLen);
        GOTO_END;
        bn_Copy(PP_W(0), L_Temp1, ModLen);
        for(j = 1; j < ((unsigned int)1 << (WindowSize - 1)); j++) {
            bn_KaraMul(L_Temp1, PP_W(j - 1), L_Temp2, ModLen);
            ret = Algorithm_REDC(L_Temp1, 2 * ModLen, L_Modulus, ModLen);
            GOTO_END;
        ...
        for(; i != (unsigned int)-1; i--) {
            ...
            if(i == (unsigned int)Add_Chain[j][0]) {
    ...
    if(Algorithm_REDC == Montgomery_REDC) {
        ...
        ret = Classical_REDC(P1, ModLen + 1, L_Modulus, ModLen);
        GOTO_END;
    }
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
unsigned int BN_Copy(BIGNUM* BN_Dst,
                     BIGNUM* BN_Src) {
    unsigned int i;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
unsigned int BN_Rand(BIGNUM* BN_Dst,
                     unsigned int BitLen) {
    unsigned int i, j;
    ...
    for(i = 0; i < BitLen / BitsInDIGIT; i++) {
        temp_rand = rand();
        BN_Dst->pData[i] = rand() ^ (temp_rand << 11) ^ (temp_rand << 19);
    }
    ...
    if(j) {
        temp_rand = rand();
        BN_Dst->pData[i] = rand() ^ (temp_rand << 11) ^ (temp_rand << 19);
        BN_Dst->pData[i] &= (((unsigned int)1) << j) - 1;
        i++;
    }
    /*
    if(j){
        BN_Dst->pData[BitLen/BitsInDIGIT-1] |= ((unsigned int)1<<(j-1));
    }
    else{
        BN_Dst->pData[BitLen/BitsInDIGIT-1] |= 0x80000000;
    }*/

    //
    // BN_Dst->pData[(BitLen-1)/BitsInDIGIT] |= 0X80000000;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
unsigned int BN_SHL(BIGNUM* BN_Dst,
                    BIGNUM* BN_Src,
                    unsigned int NumOfShift) {
    unsigned int i;
    unsigned int t;
    ...
    if(t) {
        ...
        CheckOutput_MemLen(BN_Dst);
        for(i = BN_Dst->Length - t - 1; i != (unsigned int)-1; i--)
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
unsigned int BN_SHR(BIGNUM* BN_Dst,
                    BIGNUM* BN_Src,
                    unsigned int NumOfShift) {
    unsigned int i;
    unsigned int t;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
unsigned int BN_Add(BIGNUM* BN_Dst,
                    BIGNUM* BN_Src1,
                    BIGNUM* BN_Src2) {
    unsigned int tmp;
    unsigned int carry;
    ...
    if(BN_Src1->Length == 0)
        return BN_Copy(BN_Dst, BN_Src2);
    if(BN_Src2->Length == 0)
        return BN_Copy(BN_Dst, BN_Src1);
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
unsigned int BN_Sub(BIGNUM* BN_Dst,
                    BIGNUM* BN_Src1,
                    BIGNUM* BN_Src2) {
    unsigned int tmp;
    unsigned int carry;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
unsigned int BN_Mul(BIGNUM* BN_Dst,
                    BIGNUM* BN_Multiplicand,
                    BIGNUM* BN_Multiplier) {
 ...
 */
unsigned int BN_Div(BIGNUM* BN_Quotient,
                    BIGNUM* BN_Remainder,
                    BIGNUM* BN_Dividend,
                    BIGNUM* BN_Divisor) {
    unsigned int tmp;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
unsigned int BN_ModAdd(BIGNUM* BN_Dst,
                       BIGNUM* BN_Src1,
                       BIGNUM* BN_Src2,
                       BIGNUM* BN_Modulus) {
    unsigned int tmp;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
unsigned int BN_ModSub(BIGNUM* BN_Dst,
                       BIGNUM* BN_Src1,
                       BIGNUM* BN_Src2,
                       BIGNUM* BN_Modulus) {
    unsigned int tmp;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
unsigned int BN_ModRed(BIGNUM* BN_Dst,
                       BIGNUM* BN_Src,
                       BIGNUM* BN_Modulus) {
    unsigned int i;
    ...
    //
    for(i = 0; i < BN_Src->Length; i++)
        Value[i] = BN_Src->pData[i];
    ...
    for(i = 0; i < BN_Modulus->Length; i++)
        BN_Dst->pData[i] = Value[i];
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
unsigned int BN_ModMul(BIGNUM* BN_Dst,
                       BIGNUM* BN_Src1,
                       BIGNUM* BN_Src2,
                       BIGNUM* BN_Modulus) {
    unsigned int i;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
unsigned int BN_ModInv(BIGNUM* BN_Dst,
                       BIGNUM* BN_Src,
                       BIGNUM* BN_Modulus) {
    unsigned int i;
    unsigned int ret;
    ...
    //
    for(i = 0; i < BN_Src->Length; i++)
        BN_Temp[i] = BN_Src->pData[i];
    for(; i < BN_Modulus->Length; i++)
        BN_Temp[i] = 0;
 ...
 *		- CTR_SUCCESS					함수가 성공적으로 수행됨.
 */
unsigned int BN_ModExp(BIGNUM* BN_Dst,
                       BIGNUM* BN_Base,
                       BIGNUM* BN_Exponent,
                       BIGNUM* BN_Modulus) {
...
unsigned int Classical_REDC(unsigned int* L_Dst,
    ...
    //	Step 2 : set MSB
    for(i = BitsInDIGIT - 1; i != (unsigned int)-1; i--) {
        if(L_Modulus[ModLen - 1] & ((unsigned int)1 << i))
            break;
        MSB++;
    }
    if(MSB) {
...
unsigned int MillerRabin(
    ...
    ret = CTR_VERIFY_FAIL;
    if(BN_Num->Length == 0)
        goto LABEL_END; // 길이가 0이면
    if(isEven0(BN_Num->pData))
        goto LABEL_END; // 입력 BIGNUM이 짝수이면
    ...
    if((BN_Num_1 = CreateBigNum(DigitLen + 1)) == NULL)
        goto LABEL_END;
    if((BN_Tmp = CreateBigNum(DigitLen + 1)) == NULL)
        goto LABEL_END;
    if((T = CreateBigNum(DigitLen + 1)) == NULL)
        goto LABEL_END;
    if((M = CreateBigNum(DigitLen + 1)) == NULL)
        goto LABEL_END;
    ...
    for(i = 0; i <= NoTest; i++) {
        ...
        //
        if((BN_Cmp(M, &BN_One) == 0) || (BN_Cmp(M, BN_Num_1) == 0))
            continue;
        ...
        ret = CTR_VERIFY_FAIL;
        if(s == j)
            goto LABEL_END; // return CTR_VERIFY_FAIL;
    }
...
/*	if(ret==CTR_SUCCESS)
    {
        for(j = BN_Tmp->Length-1; j>=0; j--)
            printf("%08x ", BN_Tmp->pData[j]);
    }*/
LABEL_END:
    if(BN_Num_1 != NULL)
        DestroyBigNum(BN_Num_1);
    if(BN_Tmp != NULL)
        DestroyBigNum(BN_Tmp);
    if(T != NULL)
        DestroyBigNum(T);
    if(M != NULL)
        DestroyBigNum(M);
