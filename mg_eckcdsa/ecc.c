...
unsigned char char_bit_trancator(unsigned char IN,
                                 int bitlen) {
    if(bitlen == 1)
        return IN & 1;
    else if(bitlen == 2)
...
SINT ECC_init_params(ECC_FIELD_TYPE ftype,
                     ECC_PARAMS* ecc_params,
                     const unsigned int ECC_ID) // 초기화
{
...
SINT ECC_set_params(ECC_ID ecc_id,
                    ECC_PARAMS* ecc_params) // ecc id 는 커브의 분류 번호 , 분류 번호와 선언한 파라미터를 입력하여 파라미터를 세팅하는 함수 // ECC_initialize 이후에 시행해야 멤카피 에러가 안남
{
...
/*** PRNG ECKCDSA ***/
SINT Bit_Truncation(unsigned long* IN,
                    const int Truncater_bit_loc) {
...
void PRNG_ECKCDSA_SHA224(const unsigned char* SEED,
                         const SINT SEED_char_LENGTH,
                         unsigned char* output,
                         const SINT output_bit_LENGTH) // output should be initialized
{
    ...
    unsigned char temp_out[200] = {
        0,
    };
    unsigned char SEED_i[200] = {
        0,
    };
    unsigned char Toutput[200] = {
        0,
    };
...
void PRNG_ECKCDSA_SHA256(const unsigned char* SEED,
                         const SINT SEED_char_LENGTH,
                         unsigned char* output,
                         const SINT output_bit_LENGTH) // output should be initialized
{
