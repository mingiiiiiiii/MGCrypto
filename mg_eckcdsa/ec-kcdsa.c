...
void Byte_to_Prime_Field(unsigned char* C,
                         unsigned int* Field,
                         int Byte_length,
                         int Field_length) {
    int i;
    unsigned char Converted_C[100] = {
        0,
    };
...
void Word_to_Byte(unsigned int* W,
                  unsigned char* C,
                  int W_length,
                  int char_length) {
    int i;
...
void Point_to_Byte(unsigned int* W,
                   unsigned char* C,
                   int W_length,
                   const int Temp_char_length) {
    int i;
    unsigned char TEMP[100] = {
        0,
    };
...
void eckcdsa_gfp(const ECC_POINT Private,
                 const ECC_PARAMS SELECTED_CURVE,
                 unsigned int* secret,
                 const unsigned int ECC_ID,
                 const unsigned int HASH_FUNCTION,
    ...
    UINT e_dat[MAX_GF_BUF_LEN] = {
        0,
    },
         s_dat[MAX_GF_BUF_LEN], k_e_dat[MAX_GF_BUF_LEN];
    unsigned char r_xor_v_dat[MAX_GF_BUF_LEN] = {
        0,
    };

    unsigned int d_inv_dat[MAX_GF_BUF_LEN];
    unsigned char char_cQ_x[100] = {
        0,
    },
                  char_cQ_y[100] = {
                      0,
    ...
    unsigned int KVAL_dat[100] = {
        0,
    },
                 K_dat[100] = {
                     0,
                 };
    unsigned char KVAL_char_dat[500] = {
        0,
    };
    MPZ K, KVAL;
    MPZ d_inv, d;
    ...
    for(i = Temp_char_length * 2; i < HASH_INPUT_LENGTH; i++)
        c_Q_M[i] = 0;
    for(i = EC_KCDSA_HAS160_INPUT_BYTE_LENGTH; i < HASH_INPUT_LENGTH + MSG_len; i++)
        c_Q_M[i] = Message[i - HASH_INPUT_LENGTH];
    ...
    for(i = 0; i < HASH_OUTPUT_LENGTH; i++)
        r_xor_v_dat[i] = Hash_out_r[i] ^ Hash_out_v[i];
...
void eckcdsa_gf2n(const ECC_POINT Private,
                  const ECC_PARAMS SELECTED_CURVE,
                  unsigned int* secret,
                  const unsigned int ECC_ID,
                  const unsigned int HASH_FUNCTION,
    ...
    UINT kG_x_dat[MAX_GF_BUF_LEN] = {
        0,
    },
         kG_y_dat[MAX_GF_BUF_LEN] = {
             0,
         },
         xx_dat[MAX_GF_BUF_LEN] = {
             0,
         },
         yy_dat[MAX_GF_BUF_LEN] = {
             0,
         };
    ...
    unsigned char r_xor_v_char_dat[MAX_GF_BUF_LEN] = {
        0,
    };

    unsigned int r_xor_v_dat[MAX_GF_BUF_LEN], KVAL_dat[100] = {
                                                  0,
                                              },
                                              K_dat[100] = {
                                                  0,
    ...
    for(i = 0; i < Nbyte; i++)
        c_Q_M[i] = char_cQ_x[i];
    for(i = Nbyte; i < Nbyte * 2; i++)
        c_Q_M[i] = char_cQ_y[i - Nbyte];
    for(i = Nbyte * 2; i < HASH_INPUT_LENGTH; i++)
        c_Q_M[i] = 0;
    for(i = EC_KCDSA_HAS160_INPUT_BYTE_LENGTH; i < HASH_INPUT_LENGTH + MSG_len; i++)
        c_Q_M[i] = Message[i - HASH_INPUT_LENGTH];
    ...
    // e = r ^ v mod n 계산
    for(i = 0; i < HASH_OUTPUT_LENGTH; i++)
        r_xor_v_char_dat[i] = Hash_out_r[i] ^ Hash_out_v[i];
...
int eckcdsa_gfp_verify(const ECC_POINT Private,
                       unsigned char signature[],
                       const ECC_PARAMS SELECTED_CURVE,
                       const unsigned int ECC_ID,
                       const unsigned int HASH_FUNCTION,
    ...
    UINT e_dat[MAX_GF_BUF_LEN] = {
        0,
         ...
         s_dat[MAX_GF_BUF_LEN];
    unsigned char r_xor_v_dat[MAX_GF_BUF_LEN] = {
        0,
    };
    unsigned char rr_dat[100] = {
        0x00,
    };
    unsigned int ss_dat[100] = {
        0x00,
    };
    ...
    if(count > 0) {
        for(i = 0; i < count; i++)
            Hash_out_v[i] = 0x00;
        for(i = HASH_OUTPUT_LENGTH - 1; i >= count; i--)
            rr_dat[i] = rr_dat[i - count];
        for(; i >= 0; i--)
            rr_dat[i] = 0x00;
    }
    ...
    // e' = r' ^ v' mod n 계산
    for(i = 0; i < HASH_OUTPUT_LENGTH; i++)
        r_xor_v_dat[i] = rr_dat[i] ^ Hash_out_v[i];
...
int eckcdsa_gf2n_verify(const ECC_POINT Private,
                        unsigned char signature[],
                        const ECC_PARAMS SELECTED_CURVE,
                        const unsigned int ECC_ID,
                        const unsigned int HASH_FUNCTION,
    ...
    // e' = r' ^ v' mod n 계산
    for(i = 0; i < HASH_OUTPUT_LENGTH; i++)
        r_xor_v_char_dat[i] = rr_dat[i] ^ Hash_out_v[i];
...
void Private_Key_generator(MPZ* Private_key,
                           ECC_PARAMS SELECTED_CURVE,
                           const ECC_ID ecc_id,
                           const int HASH_FUNCTION,
                           unsigned char* URAND_VAL,
                           const int URAND_len) {
    unsigned int tmp_XKEY[9] = {
        0x00,
    };
    unsigned int XSEED_dat[9] = {
        0x00,
    },
                 final_KEY_dat[100] = {
                     0,
                 };
    unsigned int XVAL_dat[9] = {
        0x00,
    },
                 G_XVAL_2u_dat[100] = {
                     0,
    ...
    unsigned char G_XVAL_2u_char_dat[500] = {
        0,
    };
    unsigned char XVAL_char_dat[36] = {
        0x00,
    };
    unsigned char XSEED_char_dat[36] = {
        0x00,
    };
...
void Public_Key_generator_gfp(ECC_POINT* Public_point,
...
void Public_Key_generator_gf2n(ECC_POINT* Public_point,
                               unsigned int* Private_key,
                               ECC_PARAMS SELECTED_CURVE) {
    unsigned int d_inv_dat[MAX_GF_BUF_LEN] = {
        0,
...
void Public_Key_generator(ECC_POINT* Public_point,
                          unsigned int* Private_key,
                          ECC_PARAMS SELECTED_CURVE) {
...
void EC_KCDSA_sign(const ECC_POINT Public_point,
                   const ECC_PARAMS SELECTED_CURVE,
                   unsigned int* secret,
                   const unsigned int ECC_ID,
                   const unsigned int HASH_FUNCTION,
...
int EC_KCDSA_verify(const ECC_POINT Public_point,
                    unsigned char signature[],
                    const ECC_PARAMS SELECTED_CURVE,
                    const unsigned int ECC_ID,
                    const unsigned int HASH_FUNCTION,
...
void Public_Key_set_params(ECC_POINT* Public_point,
                           const ECC_PARAMS SELECTED_CURVE,
                           const unsigned int* x_Q,
                           int x_Q_len,
                           const unsigned int* y_Q,
                           int y_Q_len) {
    if(SELECTED_CURVE.field_type == ECC_PRIME_FIELD) {
        Public_point->pfield_point.x.dat = (unsigned int*)malloc(MAX_GF_BUF_LEN * sizeof(unsigned int));
        Public_point->pfield_point.y.dat = (unsigned int*)malloc(MAX_GF_BUF_LEN * sizeof(unsigned int));
        ...
        for(int i = 0; i < x_Q_len; ++i)
            Public_point->pfield_point.x.dat[i] = x_Q[i];
        for(int i = 0; i < y_Q_len; ++i)
            Public_point->pfield_point.y.dat[i] = y_Q[i];
    } else if(SELECTED_CURVE.field_type == ECC_CHAR2_FIELD) {
        Public_point->c2field_point.x = (unsigned int*)malloc(MAX_GF_BUF_LEN * sizeof(unsigned int));
        Public_point->c2field_point.y = (unsigned int*)malloc(MAX_GF_BUF_LEN * sizeof(unsigned int));
        ...
        for(int i = 0; i < x_Q_len; ++i)
            Public_point->c2field_point.x[i] = x_Q[i];
        for(int i = 0; i < y_Q_len; ++i)
            Public_point->c2field_point.y[i] = y_Q[i];
    } else
        return;
}
