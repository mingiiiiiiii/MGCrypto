...
int AES_set_decrypt_key(const unsigned char* userKey,
    ...
    status = AES_set_encrypt_key(userKey, bits, aes_key);
    if(status < 0) {
        return status;
    }
    ...
    for(i = 0, j = 4 * (aes_key->round); i < j; i += 4, j -= 4) {
        temp = rk[i];
        rk[i] = rk[j];
        rk[j] = temp;
        temp = rk[i + 1];
