...
int32_t CTR_DRBG_Test_PR_useDF(void) {
    ...
    ret = MG_Crypto_CTR_DRBG_Instantiate(&state, ALGO_AES128, EntropyInput, 16, Nonce, 8, PersStr, 16, USE_DERIVATION_FUNCTION);
    ...
    ret = MG_Crypto_CTR_DRBG_Reseed(&state, EntropyInputPR1, 16, AddInput1, 16);
    ret = MG_Crypto_CTR_DRBG_Generate(&state, buffer, 512, NULL, 0);
    ...
    ret = MG_Crypto_CTR_DRBG_Reseed(&state, EntropyInputPR2, 16, AddInput2, 16);
    ret = MG_Crypto_CTR_DRBG_Generate(&state, buffer, 512, NULL, 0);
    // print_hex("Key ->", state.Key, 16);
    // print_hex("V   ->", state.V, 16);
