...
int keccak_absorb(uint8_t* input,
                  int inLen,
                  int rate,
                  int capacity) {
    uint8_t* buf = input;
...
int keccak_squeeze(uint8_t* output,
                   int outLen,
                   int rate,
                   int suffix) {
    uint8_t* buf = output;
...
int sha3_update(uint8_t* input,
                int inLen) {
    return keccak_absorb(input, inLen, keccakRate, keccakCapacity);
}

int sha3_final(uint8_t* output,
               int outLen) {
    int ret = 0;
